...
int main() {
    ...
    std::vector<std::string> testSubstring1 = { "basketball", "hockey", "100" };
    bool resultSubstring1 = dfa1.checkString(testSubstring1);
    std::cout << "Подстрока 1 " << (resultSubstring1 ? "принимается" : "отклоняется") << " автоматом." << std::endl;

    // Пример проверки второй подстроки
    std::vector<size_t> testSubstring2 = { '+', 20, '+' };
    bool resultSubstring2 = dfa2.checkString(testSubstring2);
    std::cout << "Подстрока 2 " << (resultSubstring2 ? "принимается" : "отклоняется") << " автоматом." << std::endl;
